...
export const fetchMembersFromRubric = async (): Promise<Member[]> => {
  ...
  if (!settings.qpayUrl || !settings.qpaySessionId || !settings.qpayEmail) {
    throw new Error(
      "QPay/Rubric API settings are not configured. Please configure them in Admin > Settings.",
    );
  }
  ...
  if (
    ...
    throw new Error(
      `Rubric API error: ${response.data.error || response.data.usererror || "Unknown error"}`,
    );
  }
  ...
  if (!response.data.allMemberships || !Array.isArray(response.data.allMemberships)) {
    throw new Error(
      `Expected allMemberships array from Rubric API, got: ${typeof response.data.allMemberships}`,
    );
  }
...
export const updateAllMembers = async () => {
  ...
  const emailToIdMap = new Map(existingMembers.map((m) => [m.email.toLowerCase(), m.id]));
...
export const syncMembershipPayments = async () => {
  ...
  const paymentKeys = new Set(existingPayments.map((p) => `${p.memberId}-${p.transactionId}`));
